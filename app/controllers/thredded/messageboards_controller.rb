...
module Thredded
  class MessageboardsController < Thredded::ApplicationController
    ...
    def index
      ...
      render json: MessageboardGroupViewSerializer.new(@groups).serializable_hash.to_json, status: 200
    end

    def show
      @messageboard = Thredded::Messageboard.friendly_find!(params[:id])
      render json: MessageboardSerializer.new(@messageboard, include: [:messageboard_group, :last_user, :last_topic]).serializable_hash.to_json, status: 200
    end
    ...
    def create
      ...
      if Thredded::CreateMessageboard.new(@new_messageboard, thredded_current_user).run
        render json: MessageboardSerializer.new(@new_messageboard, include: [:messageboard_group, :last_user, :last_topic]).serializable_hash.to_json, status: 201
      else
