...
module Thredded
  class ThemePreviewsController < Thredded::ApplicationController
    def show # rubocop:disable Metrics/MethodLength
      ...
      @topics = Thredded::TopicsPageView.new(@user, @messageboard.topics.send(Kaminari.config.page_method_name, 1).limit(3))
      ...
      @posts = Thredded::TopicPostsPageView.new(@user, topic, topic.posts.send(Kaminari.config.page_method_name, 1).limit(3))
      ...
      @private_posts = Thredded::TopicPostsPageView.new(@user, private_topic, private_topic.posts.send(Kaminari.config.page_method_name, 1).limit(3))
