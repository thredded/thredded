...
module Thredded
  ...
  describe UserTopicRead, '#find_or_create_by_user_and_topic' do
    ...
    it 'finds an existing record' do
      user = create(:user)
      topic = create(:topic)
      existing_topic_read = create(:user_topic_read,
        topic_id: topic.id,
        user_id: user.id)
      user_topic_read =
        UserTopicRead.find_or_create_by_user_and_topic(user, topic, '1')

      user_topic_read.should eq existing_topic_read
    end
    ...
    describe 'creating new read status for a 5 post pagination on a 7 post topic' do
      ...
      it 'sets it to the 1st post on page 1' do
        ...
        user_topic_read.post_id.should eq first_post.id
        user_topic_read.posts_count.should eq 1
        user_topic_read.page.should eq 1
      end
      ...
      it 'sets it to the 7th post in total, 1st post on that page, in page 2' do
        ...
        user_topic_read.post_id.should eq sixth_post.id
        user_topic_read.posts_count.should eq 5
        user_topic_read.page.should eq 2
      end
    end
  end
  ...
  describe UserTopicRead, '.update_read_status!' do
    describe '5 post pagination. Was page 1, 3 posts.' do
      before do
        ...
        @page = 1
        @old_read = create(:user_topic_read,
          topic_id: @topic.id,
          user_id: @user.id,
          post_id: @topic.posts.last.id,
          posts_count: 3,
          page: @page)
      end
      ...
      it 'now on page 1, 7 posts total - sets to page 1, 5th post' do
        ...
        new_read.post_id.should eq fifth_post.id
        new_read.page.should eq 1
        new_read.posts_count.should eq 5
      end
      ...
      it 'now on page 2, 7 posts total - sets to page 2, 7th post' do
        ...
        new_read.post_id.should eq seventh_post.id
        new_read.page.should eq 2
        new_read.posts_count.should eq 7
      end
    end
